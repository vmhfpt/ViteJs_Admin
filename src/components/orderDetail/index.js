...
export default function OrderDetail(params){
    const renderList = () => {
        ...
        Promise.all([
            orderDetailService.findOne(params.id),
            orderService.findOne(params.id),
            productService.index()
        ])
        .then(([dataOrderDetail, dataOrder, dataProducts]) => {
            dataProducts.map((item, key) => {
                $('#product-select').append(`<option value="${item._id}/${item.price_sale}">${item.name} -- ${new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(item.price_sale)} </option>`)
            })
            let total = 0;
            $('.title-order').empty();
            $('.title-order').html(` <span class="text-muted fw-light ">Home/</span> Detail order "${dataOrder.name}"`)
            ...
            <td>
              <div class="dropdown">
                ...
                <div class="dropdown-menu">
                  ...
                  <a data-delete="${item._id}" onclick="confirmDelete(this);" class="dropdown-item confirm-delete" href="javascript:void(0);"><i class="bx bx-trash me-1"></i> Delete</a>
                </div>
              </div>
            </td>
          </tr>`);
      ...
      window.addProductToOrder = () => {
        ...
        orderDetailService.create(dataProdctAdd)
        .then((data) => {
            renderList();
